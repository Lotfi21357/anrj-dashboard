...
# On définit des tickers principaux + alternatives si échec
POSITIONS = [
    {"nom": "MSCI World AV",  "ticker_main": "MWRD.L",  "ticker_fallback": "IWDA.AS", "parts": 36.33, "prm": 145.09},
    {"nom": "MSCI World PEA", "ticker_main": "DCAM.PA", "ticker_fallback": None,       "parts": 481,   "prm": 5.261},
    {"nom": "Global Hydrogen","ticker_main": "ANRJ.PA", "ticker_fallback": None,        "parts": 4.77, "prm": 706.06},
    {"nom": "EM Asia",        "ticker_main": "AASI.PA", "ticker_fallback": None,        "parts": 40.83, "prm": 52.48},
    {"nom": "Or Physique",    "ticker_main": "CGLD.PA", "ticker_fallback": "GLD",      "parts": 4.59, "prm": 163.39},
]
...
def download_with_fallback(ticker, start):
    """Essaie de télécharger ticker, et si échec ou vide, tente le fallback s'il existe."""
    # Premier essai
    try:
...
def load_all_data():
    all_main = [p["ticker_main"] for p in POSITIONS]
    all_fallback = {p["ticker_main"]: p["ticker_fallback"] for p in POSITIONS if p["ticker_fallback"]}
    all_tickers = list(set(all_main + [BENCHMARK_TICKER] + EXTRA_TICKERS))
    start = datetime.now() - timedelta(days=400)
    data = {}

    # Téléchargement groupé (peut échouer partiellement)
    try:
    ...
    # Pour les tickers absents ou vides, essai individuel + fallback
    for t in all_tickers:
        if t not in data or data[t].empty:
            # essai individuel du ticker principal
            df = yf.download(t, start=start, progress=False)
            if not df.empty:
                data[t] = df
            elif t in all_fallback:
                # fallback
                fb = all_fallback[t]
                df_fb = yf.download(fb, start=start, progress=False)
                if not df_fb.empty:
                    data[t] = df_fb  # on garde le ticker principal comme clé, mais les données viennent du fallback
    return data
...
# -------- CALCULS AVEC GESTION DES DONNÉES MANQUANTES --------
positions_calculees = []
missing_positions = []
valeur_totale = 0.0
for pos in POSITIONS:
    ticker = pos["ticker_main"]
    prix = latest_prices.get(ticker)
    if prix is None or np.isnan(prix):
        missing_positions.append(pos["nom"])
        prix = None
        valeur = 0.0
        perf = None
    else:
...
# Si on a des manquants, la valeur totale est sous-estimée (ne contient pas ces lignes)
if missing_positions:
    st.warning(f"⚠️ Données indisponibles pour : {', '.join(missing_positions)}. La valeur totale et le gain net sont partiels.")
...
def evaluate_hydrogen():
    ...
    if anrj_sma50_now and anrj_current < anrj_sma50_now:
        if anrj_series is not None and len(anrj_series) >= 5 and anrj_sma50 is not None:
            ...
            if len(last5_sma) >= 5 and all((pd.notna(s) and v < s) for v, s in zip(last5, last5_sma)):
...
if anrj_current and not np.isnan(anrj_current):
    d1,d2,d3 = st.columns(3)
    d1.metric("ANRJ", f"{anrj_current:.2f}€")
    d2.metric("SMA50", f"{anrj_sma50_now:.2f}€" if anrj_sma50_now and not np.isnan(anrj_sma50_now) else "N/A")
    d3.metric("RSI", f"{anrj_rsi_now:.1f}" if anrj_rsi_now and not np.isnan(anrj_rsi_now) else "N/A")
    st.caption(evaluate_hydrogen()[0])
else:
    st.warning("ANRJ indisponible")

st.subheader("🌏 Détails EM Asia (AASI)")
if aasi_current and not np.isnan(aasi_current):
    e1,e2 = st.columns(2)
    e1.metric("AASI", f"{aasi_current:.2f}€")
    e2.metric("SMA50", f"{aasi_sma50_now:.2f}€" if aasi_sma50_now and not np.isnan(aasi_sma50_now) else "N/A")
    if ratio_current and not np.isnan(ratio_current):
        e3,e4 = st.columns(2)
        e3.metric("Ratio AASI/CW8", f"{ratio_current:.4f}")
        e4.metric("Ratio moy. 20j", f"{ratio_avg20_now:.4f}" if ratio_avg20_now and not np.isnan(ratio_avg20_now) else "N/A")
